...
namespace asp
{
                         ...
                         vw::Vector2 const &query_pixel)
  {
    ...
    vw::ImageViewRef<vw::PixelMask<float>> masked_dem = vw::create_mask(vw::DiskImageView<float>(dem_file), nodata_val);
    ...
    vw::Vector3 xyz = vw::cartography::camera_pixel_to_dem_xyz(cam_ctr, cam_dir, masked_dem,
                                       ...
                                       std::vector<vw::Vector2> const &query_pixels)
  {
    ...
    if (dem_file.empty())
      vw::vw_throw(vw::ArgumentErr()
                   << "The DEM file must be provided to be able to query pixels.\n");
    ...
    double nodata_val = -std::numeric_limits<float>::max();
    vw::read_nodata_val(dem_file, nodata_val);

    // Create the masked DEM - only load once for all pixels
    vw::ImageViewRef<vw::PixelMask<float>> masked_dem = vw::create_mask(vw::DiskImageView<float>(dem_file), nodata_val);

    // Store the results for each pixel
    std::vector<vw::Vector3> results;
    results.reserve(query_pixels.size());
    ...
    for (const auto &query_pixel : query_pixels)
    {
      vw::Vector3 cam_ctr = camera_model->camera_center(query_pixel);
      vw::Vector3 cam_dir = camera_model->pixel_to_vector(query_pixel);
      ...
      vw::Vector3 xyz = vw::cartography::camera_pixel_to_dem_xyz(cam_ctr, cam_dir, masked_dem, dem_georef, treat_nodata_as_zero,
                                                                 has_intersection, dem_height_error_tol, max_abs_tol, max_rel_tol,
                                                                 num_max_iter, xyz_guess, height_guess);
