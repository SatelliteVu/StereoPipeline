...
namespace asp
{
                           ...
                           TerminalProgressCallback const &tpc)
  {
    ...
    if (!opt.noGeoHeaderInfo)
    {
      ...
      ;
      if (prefix == "")
        prefix = "NONE"; // to save the field, need to make it non-empty
      ...
      vw::camera::AdjustedCameraModel *adj_cam = dynamic_cast<vw::camera::AdjustedCameraModel *>(opt.camera_model.get());
      if (adj_cam != NULL)
      {
        q = adj_cam->rotation();
                            ...
                            Map2CamTransT const &transform)
  {
    ...
    {
      write_parallel_type( // Write to the output file
          opt.output_file,
